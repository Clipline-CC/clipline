...
**Components:**
- **Capture Engine:** Automatic uses WGC on Windows 11. On Windows 10, it uses WGC for an ordinary game window and switches to DXGI Desktop Duplication when that exact foreground window's client area covers its monitor; display and region targets use Desktop Duplication. Default (Windows 11) forces WGC and Fallback mode forces the Windows 10 route. WGC still shows its yellow border around windowed apps on Windows 10. The fullscreen switch closes WGC before opening Desktop Duplication, checks the window and monitor again after each frame, and emits black during transitions or while a fullscreen game loses focus. **Exclusive-fullscreen games cannot be captured per-window without injection**: Desktop Duplication captures the whole monitor, including overlays, notifications, and other visible content. A hardware cursor may be absent. Frames stay as GPU textures and share one D3D device and capture clock with the encoder and audio.
