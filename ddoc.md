...
- **Recommendation: Tauri**, with the timeline/editor as a web canvas component — with two provisos. (1) **WebView2 cannot be relied on to decode our default codecs**: H.264 plays everywhere, but HEVC needs the paid OS codec extension and AV1 the optional AV1 Video Extension; editor preview therefore uses a native FFmpeg decode path feeding frames to the UI (§11), not an HTML `<video>` element. (2) To actually hit ShadowPlay-class idle RAM, the WebView2 view is **released when minimized to tray**. As built, the process is *kept alive but made inert*: the host window hides, the controller's `IsVisible` goes false, and the view drops to `MemoryUsageTargetLevel::Low`. Measured on a dev machine, that trims the WebView2 resident set from ~230 MB to ~40 MB and the whole process tree from ~335 MB to ~155 MB while hidden. Scripts and network connections keep running, so the tray-hidden UI still receives recorder events and the Save Replay hotkey keeps working — which is why this is preferred over `TrySuspend` (pauses scripts) and why `Low`/`Normal` and `Suspend`/`Resume` must not be mixed. **Fully destroying the webview and recreating the UI on demand remains the stronger option** and is what this section originally specified: it would reclaim the ~40 MB residue and the renderer's JS heap too, at the cost of rebuild latency on reveal and a state re-sync. Revisit if idle RAM needs to go lower.
...
- **Retention is bounded by span *and* bytes, planned together.** The byte budget carries a 2× encoder-overshoot headroom so a bitrate spike cannot evict footage the save window still needs. On its own that headroom becomes a target rather than a cap: because eviction is oldest-first by size, the ring grows until it *reaches* the budget, holding roughly twice the footage a save can use — and far more when the encoder undershoots, since fewer bytes per second means more seconds fit under the cap (measured at ~5× the intended span on low-motion content). The retention window is therefore `replay_window_s + BUFFER_HEADROOM_S`, derived rather than read from persisted settings, and the two bounds are resolved in one plan: whichever demands more eviction wins, and that single count is advanced to the next keyframe so the surviving front always starts a decodable GOP. Sequencing them separately would let the byte bound — which removes the minimum count with no keyframe awareness — strand a headless GOP at the front. Under overshoot large enough that bytes demand more eviction than retention, **bytes win**: retention falls below the requested window, because the alternative is unbounded growth.
