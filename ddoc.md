...
- **Simultaneous full-session recording + replay buffer** (Goal 2) shares one encoder session per track: encoded GOP-aligned segments fan out to two sinks (ring buffer and a best-effort writer thread for the open recording file) rather than running duplicate encoder sessions — staying within session limits on older GPUs and keeping disk stalls off the capture loop. Today this is enabled per built-in game plugin or saved custom game in Settings > Games; games set to `full_session` write `session_<unix>.mp4` while Save Replay continues to use the same encoded GOP ring. Full-session sink failures degrade to warnings and do not tear down replay capture.
