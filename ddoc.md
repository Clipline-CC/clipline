...
*the captured game executable and reads `gameData.queue.id` from the authenticated loopback
  ...
  **Per-game-type recording gate.** Settings carry a record toggle per queue category (plus an
  Unknown policy for failed lookups), defaulting to record-all. When any category is off, a newly
  detected League game tears down the previous recorder immediately and defers its replacement until
  one bounded LCU lookup (LcuClient's 2 s bound) resolves the queue. A denied game is not recorded
  and emits a skip notice; manual session recording always bypasses the gate. The verdict is cached
  per detection as Pending/Allowed/Denied and governs every automatic start path, so settings saves
  or autostart resumes cannot start a recorder for a pending or denied game; toggles changed
  mid-lookup are honored when the verdict is computed.
