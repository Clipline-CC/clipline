...
- **Audio:** WASAPI loopback for a selected system output endpoint and a separate microphone track. The withdrawn startup-only per-process experiment is not a supported capture mode. The next audio milestone replaces the singular output setting with an ordered list of explicit playback endpoints, each retained as its own Opus track for per-clip selection.
