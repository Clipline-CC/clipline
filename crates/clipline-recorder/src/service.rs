//! The shared replay-buffer service: a dedicated recorder thread (ddoc §3 — the
...
use crate::marker_source::PollerMsg;
use crate::time::{unix_now as unix_now_u64, unix_now_i64};
...
pub struct ActiveGame {
    pub identity: clipline_games::identity::GameIdentity,
    pub name: String,
}
...
impl AppSettingsServiceExt for clipline_settings::AppSettings {
    fn to_service_options(&self, lol_url: Option<String>) -> Result<ServiceOptions, String> {
    ...
    fn to_service_options_with_registry(
        ...
        Ok(ServiceOptions {
            capture_source: match self.capture_mode {
                clipline_settings::CaptureMode::PrimaryMonitor => CaptureSource::PrimaryMonitor,
                clipline_settings::CaptureMode::WindowTitle => {
                    CaptureSource::WindowTitle(self.window_title.trim().to_string())
                }
                clipline_settings::CaptureMode::DisplayRegion => {
            ...
            buffer_bytes: clipline_settings::estimated_buffer_bytes(
                self.replay_window_s,
                self.effective_bitrate_mbps(),
            ),
            replay_storage: match self.replay_storage.mode {
                clipline_settings::ReplayStorageMode::Memory => ReplayStorageOptions::Memory,
                clipline_settings::ReplayStorageMode::Disk => ReplayStorageOptions::Disk {
                    dir: clipline_settings::normalize_replay_cache_dir(
                        &self.replay_storage.disk_dir,
                    )?,
                    quota_bytes: clipline_settings::replay_cache_quota_bytes_from_gb(
                        self.replay_storage.disk_quota_gb,
                    )?,
                },
            },
            disk_quota_bytes: clipline_settings::quota_bytes_from_gb(self.disk_quota_gb)?,
            recording_mode: RecordingMode::ReplaysOnly,
...
impl From<clipline_settings::games::GameRecordingMode> for RecordingMode {
    fn from(value: clipline_settings::games::GameRecordingMode) -> Self {
        match value {
            clipline_settings::games::GameRecordingMode::FullSession => Self::FullSession,
            clipline_settings::games::GameRecordingMode::ReplaysOnly => Self::ReplaysOnly,
        }
    }
}
...
fn marker_source_kind(opts: &ServiceOptions) -> MarkerSourceKind {
    ...
    if clipline_games::plugin::has_event_source(plugin_id) {
    ...
    match marker_source_kind(opts) {
        MarkerSourceKind::Plugin => {
            let plugin_id = opts
                ...
                .as_ref()
                .and_then(|game| game.identity.plugin_id());
            crate::marker_source::spawn_for_profile(plugin_id, opts.lol_url.clone(), recording_t0)
                .expect("marker source kind checked plugin event source")
        }
        MarkerSourceKind::LegacyLeaguePoller => {
            crate::marker_source::spawn(opts.lol_url.clone(), recording_t0)
        }
    }
}
...
fn prepare_replay_storage(opts: &ServiceOptions) -> Result<PreparedReplayStorage, String> {
    match &opts.replay_storage {
        ...
        ReplayStorageOptions::Disk { dir, quota_bytes } => {
            ...
            let preserved_bytes = sweep_replay_cache_runs(dir, now, process_instance_id)?;
            ...
            let current_process_instance_id = process_instance_id(std::process::id())?;
...
fn sweep_replay_cache_runs(
    ...
    for entry in entries.flatten() {
        ...
        if !metadata.is_dir() || clipline_shell::metadata_is_link_or_reparse_point(&metadata) {
            continue;
        }
...
fn replay_cache_run_size(path: &Path) -> u64 {
    ...
    if clipline_shell::metadata_is_link_or_reparse_point(&metadata) {
        return 0;
    }
    if metadata.is_file() {
        return metadata.len();
...
fn available_space_bytes(path: &Path) -> Result<u64, String> {
    clipline_shell::windows::filesystem::available_space_bytes(path)
        .map_err(|error| format!("could not read free space for {path:?}: {error}"))
}

fn process_instance_id(process_id: u32) -> Result<String, String> {
...
fn minimum_full_session_duration_s(active_game: Option<&ActiveGame>) -> f64 {
    match active_game {
        Some(game)
            if game
                .identity
                .is_built_in_plugin(clipline_games::plugin::OSU_ID) =>
        {
            10.0
        }
        _ => 0.0,
...
pub fn clips_dir_resolved(
    media_dir: &Path,
    fallback: impl FnOnce() -> PathBuf,
) -> Result<(PathBuf, bool), String> {
    media_root::clips_dir_resolved_with_probe(
        media_dir,
        fallback,
        media_root::probe_writable_directory,
    )
}
...
pub fn prepare_writable_media_directory(dir: &Path) -> Result<(), String> {
    prepare_writable_directory_with(dir, media_root::probe_writable_directory)
...
mod tests {
    ...
    fn marker_source_uses_active_plugin_event_source_when_available() {
        let opts = ServiceOptions {
            active_game: Some(ActiveGame {
                identity: clipline_games::identity::GameIdentity::built_in_plugin(
                    clipline_games::plugin::LEAGUE_OF_LEGENDS_ID,
                )
                .unwrap(),
    ...
    fn custom_identity_cannot_enable_a_built_in_marker_source() {
        let opts = ServiceOptions {
            active_game: Some(ActiveGame {
                identity: clipline_games::identity::GameIdentity::custom(
                    clipline_games::plugin::LEAGUE_OF_LEGENDS_ID,
                ),
    ...
    fn writable_directory_probe_leaves_no_probe_file() {
        ...
        media_root::probe_writable_directory(&media).unwrap();
