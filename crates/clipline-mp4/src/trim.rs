...
fn prepare_decoded_audio_sample(
    ...
    let decoded_duration = u32::try_from(pcm.len() / 2)
        .map_err(|_| TrimError::Corrupt("decoded Opus packet is too long".into()))?;
    // Opus packets decode to complete coded frames, while the final MP4 sample
    // may carry a shorter duration to discard encoder padding at end of stream.
    // Only that bounded terminal crop is valid; intermediate mismatches and a
    // container duration longer than decoded PCM still fail closed.
    let terminal_discard_padding =
        sample.is_physical_terminal && sample.duration < decoded_duration;
    if sample.duration.abs_diff(decoded_duration) > 1 && !terminal_discard_padding {
        return Err(TrimError::Corrupt(format!(
            "MP4 and decoded Opus sample durations differ (container {}, decoded {decoded_duration})",
            sample.duration
        )));
    }
...
pub(crate) struct SampleRecord {
    ...
    /// True only for the physical final sample-table entry. Edit-list mapping
    /// may expose an earlier packet last, but that packet cannot carry Opus
    /// end-padding discard semantics.
    pub(crate) is_physical_terminal: bool,
}
    ...
    for (&chunk_offset, &chunk_samples) in chunk_offsets.iter().zip(samples_per_chunk) {
        ...
        for _ in 0..chunk_samples {
            ...
            out.push(SampleRecord {
                ...
                start_ticks,
                is_physical_terminal: sample_index + 1 == sizes.len(),
            });
...
mod tests {
    ...
    fn clipline_staggered_opus_audio_fixture() -> Vec<u8> {
        let mut w =
            HybridMp4Writer::new_multi(Cursor::new(Vec::new()), tracks_two_audio()).unwrap();
        let v = video_gop(0);
        let output = opus_audio_packets(0.20);
    ...
    fn clipline_gapped_opus_audio_fixture() -> Vec<u8> {
        let mut w =
            HybridMp4Writer::new_multi(Cursor::new(Vec::new()), tracks_two_audio()).unwrap();
        let v = video_gop(0);
        let output = opus_audio_packets(0.20);
    ...
    fn decoded_audio_consumes_per_track_pre_skip_and_quantized_durations() {
        ...
        let terminal_tail = SampleRecord {
            offset: 0,
            size: 0,
            duration: 312,
            is_sync: true,
            decode_ticks: 0,
            start_ticks: 0,
            is_physical_terminal: true,
        };
        let mut terminal_state = AudioMixTrackState {
            next_sample: 0,
            pending: None,
            previous_source_end: None,
            output_cursor: None,
        ...
        let terminal_tail = prepare_decoded_audio_sample(
            &terminal_tail,
            vec![0.0; OPUS_MIX_FRAME_TICKS as usize * 2],
            &mut terminal_state,
        )
        .unwrap()
        .unwrap();
        assert_eq!(terminal_tail.pcm.len(), 312 * 2);

        let terminal_longer_than_decoded = SampleRecord {
            offset: 0,
            size: 0,
            duration: 960,
            is_sync: true,
        ...
        let mut terminal_longer_state = AudioMixTrackState {
            next_sample: 0,
            pending: None,
            previous_source_end: None,
            output_cursor: None,
        ...
        assert!(prepare_decoded_audio_sample(
            &terminal_longer_than_decoded,
            vec![0.0; 312 * 2],
            &mut terminal_longer_state,
        )
        .is_err());

        let decoded_oversized = SampleRecord {
            duration: 960,
            is_physical_terminal: true,
            ..terminal_longer_than_decoded
        };
        assert!(prepare_decoded_audio_sample(
            &decoded_oversized,
            vec![0.0; (MAX_OPUS_FRAME_TICKS as usize + 1) * 2],
            &mut terminal_longer_state,
        )
        .is_err());
    }
