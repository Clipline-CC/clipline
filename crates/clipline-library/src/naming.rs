...
pub fn inferred_clip_kind_for_path(path: &Path) -> &'static str {
    let name = path
        .file_name()
...
pub fn normalized_clip_title(input: &str) -> Result<String, String> {
...
pub fn normalized_clip_file_name(input: &str) -> Result<String, String> {
    let trimmed = input.trim();
    if trimmed.is_empty() {
        return Err("clip name cannot be empty".into());
    }
...
pub fn is_reserved_windows_file_name(stem: &str) -> bool {
    let base = stem
        .split('.')
        .next()
        .unwrap_or_default()
