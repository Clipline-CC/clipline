//! Frontend-independent first-party supported game registry.
//!
...
impl GameProfileManifest {
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if let Some(event_source) = self.event_source.as_deref() {
            if !supported_event_source(event_source) {
    ...
    pub fn match_window<'a>(&self, windows: &'a [GameWindow]) -> Option<&'a GameWindow> {
...
impl WindowMatchRule {
    ...
    fn match_window<'a>(&self, windows: &'a [GameWindow]) -> Option<&'a GameWindow> {
...
impl GamePlugin {
    ...
    #[cfg(test)]
    fn info(&self, icon_cache_dir: &std::path::Path) -> GamePluginInfo {
        self.info_with_icon(self.icon_string(icon_cache_dir))
    }
    ...
    fn immutable_info(&self) -> GamePluginInfo {
        let icon = if self.uses_extracted_icon() {
            ...
            self.icon_string(std::path::Path::new(""))
        };
    ...
    fn info_with_icon(&self, icon: Option<String>) -> GamePluginInfo {
        ...
        GamePluginInfo {
            ...
            event_markers: self
                ...
                .event_source
                .as_deref()
                .is_some_and(supported_event_source),
    ...
    fn icon_string(&self, icon_cache_dir: &std::path::Path) -> Option<String> {
        match self.manifest.icon.as_ref() {
            ...
            Some(GameProfileIcon::Extracted) | None => {
                let cache = game_profile_icon_cache_path(icon_cache_dir, self.id())?;
...
fn game_profile_icon_cache_path(
    ...
    Some(icon_cache_dir.join(format!("{profile_id}.png")))
}
...
pub fn ensure_plugin_icon_cached(
    icon_cache_dir: &std::path::Path,
    profile_id: &str,
    exe_path: &str,
) {
    ...
    let Some(cache) = game_profile_icon_cache_path(icon_cache_dir, profile_id) else {
        return;
    };
    if cache.exists() {
        return;
    }
    if let Some(png) = extract_exe_icon_png(exe_path) {
...
pub fn plugin_id_for_game_id(game_id: GameId) -> &'static str {
    match game_id {
        ...
        GameId::Valorant => crate::identity::VALORANT_ID,
        GameId::Cs2 => crate::identity::CS2_ID,
        GameId::Osu => OSU_ID,
    }
}
...
pub fn has_event_source(profile_id: Option<&str>) -> bool {
    event_source_for_profile(profile_id).is_some()
}

pub fn event_source_for_profile(profile_id: Option<&str>) -> Option<&'static str> {
    let id = profile_id?;
    let profile = all().iter().find(|profile| profile.id() == id)?;
    match profile.manifest.event_source.as_deref()? {
        LEAGUE_LIVE_CLIENT_EVENT_SOURCE => Some(LEAGUE_LIVE_CLIENT_EVENT_SOURCE),
        _ => None,
    }
}
...
fn supported_event_source(name: &str) -> bool {
    name == LEAGUE_LIVE_CLIENT_EVENT_SOURCE
}
...
fn first_party_asset_data_url(path: &str) -> Option<String> {
    let bytes: &[u8] = match path {
        ...
        "assets/games/osu.png" => include_bytes!("../plugin-seeds/osu/assets/games/osu.png"),
...
fn extract_exe_icon_png(exe_path: &str) -> Option<Vec<u8>> {
    crate::windows::icon::extract_exe_icon_png(exe_path)
}

#[cfg(not(windows))]
fn extract_exe_icon_png(_exe_path: &str) -> Option<Vec<u8>> {
    None
}
...
mod tests {
    ...
    fn league_profile_has_no_install_state_but_keeps_presentation() {
        let icon_cache = TestDir::new("clipline-games", "league-profile-info");
        let profile = all()
            .iter()
            .find(|profile| profile.id() == LEAGUE_OF_LEGENDS_ID)
            .expect("league profile");
        let info = profile.info(icon_cache.path());
    ...
    fn osu_profile_is_registered_without_live_event_source() {
        let icon_cache = TestDir::new("clipline-games", "osu-profile-info");
        let profile = all()
            .iter()
    ...
    fn profile_records_and_immutable_catalog_data_are_cached() {
        let icon_cache = TestDir::new("clipline-games", "catalog-cache");
        ...
        let first_catalog = catalog(icon_cache.path());
        let second_catalog = catalog(icon_cache.path());
