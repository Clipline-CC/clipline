...
pub struct InstalledGameSource {
    pub app_id: u32,
    pub name: String,
...
pub fn project_discovery_sources(
    installed: Vec<InstalledGameSource>,
    ...
    candidates_from_sources(
        installed
            ...
            .map(|source| SteamApp {
                ...
                exe_name: source.exe_name,
                process_path: source.process_path,
            })
            .collect(),
        windows,
...
pub(crate) struct SteamApp {
    ...
    pub(crate) process_path: Option<PathBuf>,
}
...
pub(crate) fn candidates_from_sources<F>(
...
fn game_name_from_window(window: &GameWindow) -> String {
...
fn is_noise_window(window: &GameWindow) -> bool {
...
mod tests {
    ...
    fn window(title: &str, exe_name: &str, exe_path: Option<&str>) -> GameWindow {
    ...
    fn dedupes_against_existing_custom_games() {
        let existing = CustomGameSettings {
            ...
            recording_mode: clipline_settings::games::GameRecordingMode::ReplaysOnly,
            icon: None,
        };
        let candidates = candidates_from_sources(
            vec![steam_app(
