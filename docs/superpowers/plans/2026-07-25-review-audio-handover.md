...
fix: the clip-start fault was a backward *seek* on an audible element, whereas this is the previous
sidecar set being muted in the same tick the new set is unmuted. Muting does not flush already-queued
audio, so two *aligned* copies briefly overlap — which is why it reads as a short echo rather than a
stutter. Fixable the same way if it ever matters: let the outgoing set drain a frame before unmuting
the incoming one, rather than swapping in one tick. Not attempted; do not crossfade.

### Task 5 verification status — honest accounting

- **Done:** instrumentation removed, workspace tests and warning-denied Clippy green, and a
  by-ear confirmation on a release build that the clip-start repeat is gone.
- **Not done:** the loopback waveform comparison. This plan calls it "the only evidence that closes
  the question", and it has not been performed — the by-ear check is a practical substitute, not that
  evidence. Task 5 is therefore **not** complete by this plan's own definition.
- The four committed trace files were initially malformed: redaction ran over the CDP response
  wrapper, where the value is an *escaped* JSON string, so a `[^"]*` pattern consumed the backslash
  of the closing `\"`. They are now regenerated by decoding the inner event array first and redacting
  after, and each parses.
